...
class min_heap_pq():            #Class to executes function of priority queues
    ...
    def decrease_key_relax(self,key_value):                 #decrease_key function when 2 key value for same vertex but different weights
         for i in range(len(self.heap)):                    #for loop O(Vlogn) to find vertex match
             if (self.heap[i][0]==key_value[0]):
                 self.heap[i][1]=key_value[1]               # CORRECCIÓN: Actualizar el peso (índice 1), no el vértice
                 self.decrease_key(i)                       # CORRECCIÓN: Llamar a decrease_key con el índice 'i' correcto
                 break                                      # CORRECCIÓN: Romper el ciclo una vez encontrado el vértice
    ...
    def isEmpty(self):              #checks whether the heap is empty or not
        return self.heap_size == 0  # CORRECCIÓN: Debe devolver True si el tamaño es 0
...
class Dijkstra():               #Dijsktra algorithm can be found here
    ...
    def Dijkstra_alg(self, n, e, mat, s):                       #Function for Dijsktra main algorithm implemented
         ...
         while not self.key.isEmpty():              # CORRECCIÓN: Añadido 'not' lógico para que cicle mientras NO esté vacío
...
if __name__ == "__main__":
    # 1. Definimos los datos del grafo de prueba
    n = 5  # Número de vértices (del 1 al 5)
    e = 6  # Número de aristas
    
    # Matriz de aristas: [origen, destino, peso]
    mat = [
        [1, 2, 2],
        [1, 3, 4],
        [2, 3, 1],
    ...
    s = 1  # Vértice de origen (source)

    # 2. Instanciamos la clase
    dijkstra_solver = Dijkstra()

    # 3. Llamamos al algoritmo
    resultado = dijkstra_solver.Dijkstra_alg(n, e, mat, s)

    # 4. Imprimimos el resultado para que la terminal nos muestre algo
    print("El algoritmo ha terminado.")
    print("Formato del resultado: [distancia_minima, unique_shortest_path]")
    for i, res in enumerate(resultado):
        print(f"Hacia el vértice {i+1}: {res}")
